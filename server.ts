        ...
        if (job) {
          const desc = job.description
            ? job.description
                ...
                .substring(0, 200)
                .replace(/\n/g, " ")
            : "Trouvez les meilleures offres sur Liggeybi.";
          const rawImg =
            job.imageUrl ||
            job.logoUrl ||
          ...
          // Make sure description is HTML, as required by Google JobPosting
          const htmlDescription = job.description
          ...
          // Construct Google JobPosting Schema
          const jobPostingSchema = {
            "@context": "https://schema.org/",
            "@type": "JobPosting",
            title: job.titre || "Offre d'emploi",
            image: encodedImg,
            description: htmlDescription, // Google requires full HTML description
            identifier: {
              "@type": "PropertyValue",
            ...
            validThrough:
              job.dateExpiration &&
              !isNaN(new Date(job.dateExpiration).getTime())
            ...
            hiringOrganization: {
              ...
              logo: job.imageUrl
                ? job.imageUrl.startsWith("http")
                  ? job.imageUrl
                  : `https://www.liggeybi.com${job.imageUrl}`
                : job.logoUrl
                  ? job.logoUrl.startsWith("http")
                    ? job.logoUrl
                    : `https://www.liggeybi.com${job.logoUrl}`
                  : "https://www.liggeybi.com/logo.png",
            },
            jobLocation: {
              "@type": "Place",
              address: {
                "@type": "PostalAddress",
                addressLocality: job.lieu || "Sénégal",
          ...
          if (isProd) {
            ...
            // We can't synchronously transform it here easily without moving Vite instance creation higher up,
            // so for bots during DEV, we just return the raw template with our head injected.
            // Since bots mostly care about SSR HTML, in DEV the crawler will see raw vite index.
          }
