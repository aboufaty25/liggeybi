...
const CustomGoogleLoginButton = ({ onSuccess, onError }: { onSuccess: (res: any) => void, onError: (err: any) => void }) => {
  const login = useGoogleLogin({
    onSuccess: tokenResponse => {
      // Pour utiliser avec notre backend, on simule credential
...
export function Login() {
  ...
  const handleAppleSuccess = async (response: any) => {
    try {
      const res = await fetch('/api/auth/apple', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // L'API Apple renvoie authorization.id_token
        body: JSON.stringify({ id_token: response.authorization?.id_token || response.id_token, user: response.user })
      });
      const data = await res.json();
      if (res.ok) {
        login(data.token, data.user);
  ...
  const content = (
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <Card className="w-full max-w-md rounded-[2.5rem] border-gray-100 shadow-2xl p-4">
        <CardHeader className="text-center space-y-2">
          <CardTitle className="text-3xl font-black tracking-tighter text-gray-900 uppercase">Bienvenue</CardTitle>
          <CardDescription className="text-gray-500 font-medium">Connectez-vous pour postuler et gérer vos offres.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6 pt-4">
          {message && <div className="bg-blue-50 text-blue-700 p-3 rounded-xl text-xs font-bold text-center border border-blue-100 flex items-center justify-center gap-2"><AlertCircle className="w-4 h-4" />{message}</div>}
          {error && <div className="bg-red-50 text-red-600 p-3 rounded-xl text-xs font-bold text-center border border-red-100">{error}</div>}
          {success && <div className="bg-green-50 text-green-600 p-3 rounded-xl text-xs font-bold text-center border border-green-100">{success}</div>}
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input 
                  ...
                  className="h-12 rounded-xl pl-10 bg-gray-50 border-none transition-all focus:ring-2 focus:ring-blue-600" 
                />
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <button type="button" onClick={handleForgotPassword} className="text-xs font-bold text-blue-700 uppercase tracking-widest hover:underline">Oublié ?</button>
              </div>
              <div className="relative">
                <Input 
                  id="password" 
            ...
            <Button 
              type="submit"
              disabled={isLoading}
              className="w-full h-14 bg-blue-700 hover:bg-blue-800 text-lg font-black rounded-xl shadow-lg transition-transform hover:scale-[1.02]"
            >
              {isLoading ? <Loader2 className="animate-spin h-5 w-5" /> : <>Se Connecter <ArrowRight className="ml-2 h-5 w-5" /></>}
            </Button>
          </form>
          ...
          <p className="text-center text-sm font-medium text-gray-500">
            Nouveau sur Liggeybi ? <Link to="/inscription" className="text-blue-700 font-bold hover:underline">Créer un compte</Link>
          </p>
        </CardContent>
      </Card>
    </div>
