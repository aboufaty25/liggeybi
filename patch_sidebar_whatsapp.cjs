...
const whatsappBannerStr = `      {/* WhatsApp alert box */}
...
content = content.replace(whatsappBannerStr, `      {!hideWhatsApp && (
${whatsappBannerStr}
      )}`);

fs.writeFileSync('src/components/common/SidebarWidgets.tsx', content);

let catContent = fs.readFileSync('src/pages/CategoryPage.tsx', 'utf8');
catContent = catContent.replace('<SidebarWidgets />', '<SidebarWidgets hideWhatsApp={true} />');
fs.writeFileSync('src/pages/CategoryPage.tsx', catContent);
